...
## Generation of Brain
The brain is a fully connected neural network of sensor and motor neurons with random synapse weights. Because the network if fully connected, every sensor is connected to each motor. The neural network acts like a eighted directed graph with the neurons as nodes and the weights as the weight of each edge.
## Mutation of Bodies
